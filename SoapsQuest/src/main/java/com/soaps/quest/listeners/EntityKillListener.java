...
/**
 * Listener for entity kill quests.
 */
public class EntityKillListener implements Listener {
    ...
    /**
     * Constructor for EntityKillListener.
     * 
    ...
    /**
     * Handle entity death events for quest progress.
    ...
    public void onEntityDeath(EntityDeathEvent event) {
        ...
        for (ItemStack item : contents) {
            ...
            // Skip if quest is legacy type but not KILL
            if (!quest.hasObjectives() && quest.getType() != QuestType.KILL) {
                continue;
            }
            ...
            if (quest.hasObjectives()) {
                ...
                // Pass the LivingEntity for filter-based matching (supports ANY, HOSTILE, PASSIVE)
                if (quest.checkProgress(player, event.getEntity())) {
